...
VERSION_SOURCE = """\
{head}
#define SALIENT_MAJOR_VERSION  {major}
#define SALIENT_MINOR_VERSION  {minor}
#define SALIENT_PATCHLEVEL     {patch}

#define SALIENT_STRVERSION "{tag}"
{tail}"""


def update_version_header(args: argparse.Namespace) -> None:
    """Update version.h to use the given tag."""
    VERSION_HEADER = Path("src/SALIENT/version.h")
    match = re.match(
        pattern=r"(?P<head>.*?)\n#define SALIENT_MAJOR_VERSION.*?SALIENT_STRVERSION[^\n]*\n(?P<tail>.*)",
...
def replace_unreleased_tags(tag: str, dry_run: bool, path: Path) -> None:
    ...
    for directory, _, files in os.walk(path):
        for filename in files:
            ...
            new_text = re.sub(
                r":: *unreleased", rf":: {short_tag}", text, flags=re.IGNORECASE
            )
            if text != new_text:
